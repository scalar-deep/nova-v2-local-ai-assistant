...
| Successful venv08 runtime boot | `logs/full_runtime_demo_venv08.log` |
| Wake word works | `logs/01_wake_word.log` |
| STT works | `logs/02_stt.log`, `audio/debug_last_stt.wav`, `audio/debug_last_stt_info.txt` |
...
| Camera snapshot works | `logs/07_camera_snapshot.log`, `images/capture_test.jpg`, `images/capture_test_info.txt` |
...
- Runtime logs may include imperfect STT examples.
- Camera is used on demand and released after snapshot.
- The runtime evidence was collected using the repository-local `venv08`.
